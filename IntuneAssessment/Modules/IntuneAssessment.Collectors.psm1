...
function Get-IAGroups {
    ...
    $items = foreach ($name in $usedGroupNames.Keys) {
        ...
        # Busca detalhes do grupo pelo displayName (cache reverso já resolveu nome via id)
        # Escapa o filtro OData inteiro: nomes de grupo com & # + % quebrariam a query string
        ...
        $members = if ($g) {
            ...
            if ($mResp -and $null -ne $mResp.'@odata.count') { [int]$mResp.'@odata.count' }
            else { @(Get-IAGraphCollection -Uri "$GraphV1/groups/$($g.id)/members?`$select=id&`$top=999" -SuppressNotFound).Count }
        } else { $null }
